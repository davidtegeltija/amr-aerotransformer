...
def make_synthetic_field(
    ...
    for b in range(n_samples):
        ...
        # Thin transition zone (6 columns wide) to create strong gradient
        ...
        if channels >= 2:
            data[b, 1, :2*r_vortex, :2*c_vortex] += 0.6 * np.cos(angles)
...
class SyntheticDataset(Dataset):
    ...
    def __init__(
        ...
        raw = make_synthetic_field(n_samples=n_samples, channels=3, height=height, width=width, seed=seed)
        
        # Transpose to [N, H, W, C] for consistency with CFDDataset
        raw = raw.transpose(0, 2, 3, 1)  # (N, H, W, 3)

        self._inputs  = raw.astype(np.float32)  # [N, H, W, 3]  - all channels as input
